...
export default function Page() {
  ...
  const [players, setPlayers] = useState(["Rose", "Robin", "Gibby", "Liv"]);
  ...
  useEffect(() => {
    scope.current = createScope({ root }).add(() => {
      ...
      // Circular motion animation for each player
      document.querySelectorAll('.playerobj').forEach((el, index) => {
        const radius = 40;
        const startAngle = (index * Math.PI * 2) / players.length; // Evenly distribute players
        
        animate(el, {
          x: (progress) => Math.cos(startAngle + progress * Math.PI * 2) * radius,
          duration: 4000
          ,
          y:(progress) => Math.sin(startAngle + progress * Math.PI * 2) * radius,
  ...
  return (
    <div ref={root} className="root overflow-hidden h-screen w-screen opacity-0">
      <div className="flex flex-col items-center justify-center h-full w-full overflow-hidden">
        ...
        <div className="relative w-[170px] h-[130px]">
          {players.map((player, index) => (
            <div 
              key={index} 
              className="absolute bg-pink-400 w-10 h-10 playerobj"
              style={{
                left: '50%',
                top: '50%',
